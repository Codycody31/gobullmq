...
The `Queue` struct represents a job queue. A new queue is created using the `NewQueue` function, which takes a context, a queue name, and functional options as parameters. These options configure the Redis connection and other queue-specific settings.
...
The `QueueOptions` struct defines the available configuration options, including Redis connection details, key prefixes, and stream event maximum lengths. Functional options like `WithRedisOptions`, `WithKeyPrefix`, and `WithStreamsEventsMaxLen` are used to apply these configurations.
...
*   **`WithRedisOptions(*redis.Options)`**: Sets the Redis connection options using a `redis.Options` struct. This is the recommended way to configure the Redis connection.
...
*   **`WithLegacyRedisConfig(ip string, password string, mode int)`**: DEPRECATED: Use `WithRedisOptions` instead.

### Redis Client Initialization

The `NewQueue` function initializes the Redis client using the provided options. It supports both the recommended `redis.Options` struct and the deprecated individual fields (`RedisIp`, `RedisPasswd`, `Mode`). If no Redis options are provided, it attempts to connect to the default localhost.
...
Queue Management in `gobullmq` provides a comprehensive set of features for managing and processing jobs in a distributed environment. It offers functionalities for queue creation, job addition, queue pausing/resuming, and event handling, making it a versatile tool for building robust background processing systems. The use of Redis ensures persistence and reliability, while the functional options pattern provides a flexible way to configure the queue.
