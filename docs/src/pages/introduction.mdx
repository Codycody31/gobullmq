...
- [README.md](README.md)
- [examples/example/main.go](examples/example/main.go)
- [job.go](job.go)
- [queue.go](queue.go)
- [scripts.go](scripts.go)
- [internal/lua/addJob_lua.go](internal/lua/addJob_lua.go)
</details>
...
A new queue can be created using the `NewQueue` function. This function takes the queue name and a set of functional options as arguments. The functional options allow you to configure the queue's behavior, such as setting the Redis connection options and key prefix.
...
| ---------------------------- | -------------------------------------------------------------------------- | --------------------------------------- |
| `WithRedisOptions(*redis.Options)` | Sets the Redis connection options using a `redis.Options` struct.        |                            |
...
A new worker can be created using the `NewWorker` function. This function takes the queue name, worker options, a Redis client, and a worker process function as arguments. The worker options allow you to configure the worker's behavior, such as setting the concurrency and stalled interval.
...
Queue events can be created using the `NewQueueEvents` function. This function takes the queue name and queue events options as arguments. The queue events options allow you to configure the queue events, such as setting the Redis client and whether to automatically start listening for events.
...
The `gobullmq` library provides a comprehensive set of tools for managing job queues in Golang applications. By leveraging Redis for storage and providing features such as worker processing and event handling, `gobullmq` simplifies the implementation of background tasks and distributed processing. The library's flexible configuration options and well-defined API make it a valuable asset for developers building scalable and reliable applications.
