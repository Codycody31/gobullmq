...
*   **`WithRedisOptions(*redis.Options)`**: Sets the Redis connection options. This is the recommended way to configure the Redis connection.
...
Customization in this project's job queue system is achieved through a combination of functional options and configuration structs. These mechanisms enable developers to tailor the behavior of queues, jobs, workers, and queue events to suit the specific needs of their applications. By leveraging these customization options, developers can optimize the performance, reliability, and flexibility of their job processing pipelines.
